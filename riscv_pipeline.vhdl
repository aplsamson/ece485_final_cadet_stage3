...
begin
    ...
    -- IF units
    -- decode instruction
    opcode <= instr(6 downto 0); -- if_id_instr(6 downto 0);
    -- Control unit
    ...
    alu_control_inst: alu_control
            port map (
                funct3 => instr(14 downto 12), -- CHANGE
                funct7 => instr(31 downto 25), -- CHANGE
                alu_op => alu_op
            );
...
--------------------------------------------------------------------------------
    ...
    reg_file_inst: reg_file
        port map (
            ...
            rs1       => if_id_instr(19 downto 15), --- CHANGE if_id_instr(19 downto 15)
            rs2       => if_id_instr(24 downto 20), --- CHANGE if_id_instr(24 downto 20)
            rd        => mem_wb_rd,
            data_in   => wb_data,
            data_out1 => reg1_data,
            data_out2 => reg2_data
...
----------------------------------------------------------------------------------------
    ...
    next_pc <=  std_logic_vector(signed(ex_mem_npc) + signed(ex_mem_imm)) when (ex_mem_branch = '1' and not_equal_flag = '1') else -- branch case
                ...
                pc when (start_stall = '1' or stall_counter = 3 or stall_counter = 2) else   -- stall case --pc when (stall = '1' or start_stall = '1') else
